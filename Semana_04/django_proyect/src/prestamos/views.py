...
def prestamos_home(request):
    return render(
        ...
        "prestamos/home.html",
        {
            ...
            "total_autorizaciones": AutorizacionEspacio.objects.count(),
        },
    )
...
def espacio_list(request):
    espacios = (
        Espacio.objects.all()
        .prefetch_related("autorizaciones__solicitante")
...
def solicitante_list(request):
    ...
    solicitantes = Solicitante.objects.select_related("perfil").prefetch_related(
...
def autorizacion_list(request):
    autorizaciones = AutorizacionEspacio.objects.select_related(
        "solicitante", "espacio"
    ).order_by("-fecha_inicio")
    return render(
        request,
        "prestamos/autorizacion_list.html",
        {"autorizaciones": autorizaciones},
    )


def autorizacion_create(request):
    form = AutorizacionEspacioForm(request.POST or None)
    if request.method == "POST" and form.is_valid():
        form.save()
        return redirect("autorizacion_list")
    return render(
        request,
        "prestamos/form.html",
        {
            "form": form,
            "titulo": "Registrar autorización de espacio",
            "url_cancelar": "autorizacion_list",
        },
    )


def autorizacion_update(request, pk):
    autorizacion = get_object_or_404(AutorizacionEspacio, pk=pk)
    form = AutorizacionEspacioForm(request.POST or None, instance=autorizacion)
    if request.method == "POST" and form.is_valid():
        form.save()
        return redirect("autorizacion_list")
    return render(
        ...
        {
            "form": form,
            "titulo": "Editar autorización de espacio",
            "url_cancelar": "autorizacion_list",
        },
    )
